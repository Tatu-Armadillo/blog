...
public class NewsController {
        ...
        @Operation(summary = "responsible for list new news ordered by creation date", description = "list new news ordered by creation date", tags = {
                        "News" }, responses = {
                                        @ApiResponse(description = "Success", responseCode = "200", content = {
                                                        @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = NewsRecord.class)))
                                        }),
        ...
        public ResponseEntity<ResponseBasePaginado<List<NewsRecord>>> listNews(
        ...
        @Operation(summary = "Search the news by ID and its subnews", description = "Search the news by ID and its subnews", tags = {
                        "News" }, responses = {
                                        @ApiResponse(description = "Success", responseCode = "200", content = @Content(mediaType = "application/json", schema = @Schema(implementation = NewsRecordList.class))),
                                        @ApiResponse(description = "Bad Request", responseCode = "400", content = @Content),
                                        @ApiResponse(description = "Not Found", responseCode = "404", content = @Content),
                                        @ApiResponse(description = "Internal Error", responseCode = "500", content = @Content)
                        })
        public ResponseEntity<ResponseBase<NewsRecordList>> showNews(@PathVariable final Long idNews) {
